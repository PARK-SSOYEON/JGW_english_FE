...
export default function WeeklyPage() {
    ...
    useEffect(() => {
        if (!season) return
        api.get('/classes', { params: { season_id: season.id } })
    ...
    return (
        <div className="p-4 md:p-6 max-w-4xl mx-auto">
            <div className="flex items-center justify-between mb-6">
                <div>
                    <h2 className="text-xl font-bold text-gray-900">주차별 현황</h2>
            ...
            {!selectedCls ? (
                ...
                <div>
                    ...
                    <div className="flex items-center justify-between mb-4">
                        <button className="btn-secondary btn-sm" onClick={() => setWeekOffset(w => w - 1)}>
                            ‹ 이전 주차
                        </button>
                        <div className="text-center">
                            ...
                            {weekOffset === 0 && <span className="text-xs text-primary font-medium">이번 주차</span>}
                            {weekOffset < 0 && <span className="text-xs text-gray-400">{Math.abs(weekOffset)}주 전</span>}
                            {weekOffset > 0 && <span className="text-xs text-gray-400">{weekOffset}주 후</span>}
                        </div>
                        <button className="btn-secondary btn-sm" onClick={() => setWeekOffset(w => w + 1)}>
                            다음 주차 ›
                        </button>
                                    ...
                                    {retestSchedules.map(s => {
                                        ...
                                        return (
                                            <div key={s.id}
                                                ...
                                                <span className="col-span-2 font-medium text-sm text-gray-900">{s.student_name}</span>
                                        ...
                                        return (
                                            <div key={s.id}
                                                ...
                                                <span className="col-span-2 text-xs text-gray-600">
                                                    ...
                                                    {done > 0 && (
                                                        <span className="text-green-600 ml-1">({minutesToHM(done)} 완료)</span>
                                                    )}
                                                </span>
                                                <span className="col-span-2">
                                                    <span className={meta.badge}>{meta.label}</span>
