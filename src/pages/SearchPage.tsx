...
export default function SearchPage() {
    ...
    const search = async () => {
        ...
        setLoading(true)
        try {
            const {data} = await api.get('/students/search', {params: {name: query.trim(), season_id: season?.id}})
            setResults(data)
            setSearched(true)
        } catch {
                ...
                <Modal title={`${attendModal.student.name} 등원 기록`} onClose={() => setAttendModal(null)} size="sm">
                    <div className="space-y-4">
                        ...
                        <div className="flex gap-2 justify-end">
                            ...
                            <button className="btn-primary" onClick={() => recordAttendance(attendModal.student)}>기록
                            </button>
                        </div>
                    </div>
                </Modal>
